...
export function iplAuctionSummary(team, players) {
  ...
  const costliestPlayer = players.reduce((prev, curr) => (curr.price > prev.price ? curr : prev));
  const cheapestPlayer = players.reduce((prev, curr) => (curr.price < prev.price ? curr : prev));
